...
export interface BootstrapStackProps extends StackProps {
  ...
  /**
   * Numeric GitHub IDs, from `gh api repos/<owner>/<repo>` (`.owner.id` and
   * `.id`). See PosterWallsBootstrap / WeddingWebsiteBootstrap for the full
   * story on GitHub's immutable-identifier OIDC subject rollout — this repo
   * is new enough that which form it emits isn't yet proven, so the trust
   * policy below accepts both.
   */
...
export class BootstrapStack extends Stack {
  constructor(scope: Construct, id: string, props: BootstrapStackProps) {
    ...
    const role = new iam.Role(this, 'DeployRole', {
      roleName: 'ChrisBridewellInfraGithubDeploy',
